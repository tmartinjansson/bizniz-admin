...
export default function APIDebugger() {
  ...
  const [endpointToTest, setEndpointToTest] = useState("/api/companies");
  ...
  const testCustomEndpoint = async () => {
    ...
    try {
      console.log(`Testing custom endpoint: ${API_BASE_URL}${customEndpoint}`);
      const res = await fetch(`${API_BASE_URL}${customEndpoint}`);
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <button 
            ...
            style={{ padding: "8px 16px", cursor: (!customEndpoint || loading) ? "not-allowed" : "pointer" }}
          >
            Test
          </button>
        </div>
